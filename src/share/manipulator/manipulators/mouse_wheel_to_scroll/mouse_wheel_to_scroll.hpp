...
class mouse_wheel_to_scroll final : public base, public pqrs::dispatcher::extra::dispatcher_client {
public:
  mouse_wheel_to_scroll(const nlohmann::json& json,
  ...
  virtual ~mouse_wheel_to_scroll(void) {
    detach_from_dispatcher([this] {
      counter_ = nullptr;
    });
  }
    ...
    if (output_event_queue) {
      ...
      if (!from_mandatory_modifiers) {
        ...
        if (auto m = front_input_event.get_event().get_if<pointing_motion>()) {
          // Ignore non-wheel pointing motions.
          if (m->get_vertical_wheel() == 0 &&
              m->get_horizontal_wheel() == 0) {
            return manipulate_result::passed;
          }
...
private:
  ...
  void post_events(const pointing_motion& pointing_motion) {
    if (auto output_event_queue = weak_output_event_queue_.lock()) {
      ...
      // Post new pointing_motion event
      {
